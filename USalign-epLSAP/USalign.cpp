        ...
        for (chain_i=0;chain_i<xchainnum;chain_i++)
        {
		    ...
		    double L_SO = 0;
                    ...
                    if (se_opt) 
                    {
                        ...
                        soi_se_main(xa, ya, seqx, seqy, TM1, TM2, TM3, TM4, TM5,
                            ...
                            secx_bond, secy_bond, mm_opt, L_SO);
                        if (outfmt_opt>=2) 
                        {
                            Liden=L_ali=0;
                            int r1,r2;
		    ...
		    //*
                    ...
                    output_results(
                        ...
                        fname_super, i_opt, a_opt, u_opt, d_opt, mirror_opt,
                        resi_vec1, resi_vec2);
		    printf("Structure Overlap = %4.3f\n", (xlen<ylen)?(L_SO/xlen):(L_SO/ylen));
		    // * /

		    /*
		    fstream output_f;
                    output_f.open("./output.txt", ios::app);
                    output_f << n_ali8 << " " << rmsd0 << " " << ((TM1>TM2)?(TM1):(TM2)) << " " << ((xlen<ylen)?(L_SO/xlen):(L_SO/ylen)) << endl;
                    output_f.close();
		    */
...
int main(int argc, char *argv[])
{
    ...
    // fstream output_f;
    // output_f.open("./totalcputime_mode_6.txt", ios::app);
    // output_f << diff << endl;
    // output_f.close();
    return 0;
}
