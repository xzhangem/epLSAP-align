...
/* assign chain-chain correspondence with OT*/

double enhanced_greedy_search(double **TMave_mat, int *assign1_list, int *assign2_list, const int chain1_num, const int chain2_num){
	const int ITER = 50;
	...
	for (int i = 0; i < chain1_num; ++i) assign1_list[i] = -1;
	for (int j = 0; j < chain2_num; ++j) assign2_list[j] = -1;
	...
	otMat[chain1_num][chain2_num] = C;
	expMat[chain1_num][chain2_num] = exp(LMBD * otMat[chain1_num][chain2_num]);
	...
	vector<double > xp(chain1_num+1, 1);
	...
	for (int i = 0; i < chain1_num; ++i){
		if (rowIndx[i] != -1){
			rowNum += 1;
			rowScoreSum += TMave_mat[i][rowIndx[i]];
		}
	}
	...
	for (int j = 0; j < chain2_num; ++j){
		if (colIndx[j] != -1){
			colNum += 1;
			colScoreSum += TMave_mat[colIndx[j]][j];
		}
	}
	double rowPropotion = rowScoreSum / rowNum;
	double colPropotion = colScoreSum / colNum;

	bool rowAssign = true;

	if (rowScoreSum == colScoreSum && rowPropotion < colPropotion) {rowAssign = false;}
	if (rowScoreSum < colScoreSum) {rowAssign = false;}

	if (rowAssign == true){
		total_score = rowScoreSum;
		for (int i = 0; i < chain1_num; i++){
			assign1_list[i] = rowIndx[i];
			if (rowIndx[i] >= 0) assign2_list[rowIndx[i]] = i;
		}
	}else{
		total_score = colScoreSum;
		for (int j = 0; j < chain2_num; j++){
			assign2_list[j] = colIndx[j];
			if (colIndx[j] >= 0) assign1_list[colIndx[j]] = j;
		}
	}


	/* Free points */
        //DeleteArray(&otMat, chain1_num+1);
	//DeleteArray(&expMat, chain1_num+1);
	//DeleteArray(&stochasticMat, chain1_num+1);
	//DeleteArray(&stochasticMat_T, chain2_num+1);
