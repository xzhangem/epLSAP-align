...
inline double maxvalue(vector<vector<double> > mat, const int rows, const int cols){
	double vmax = mat[0][0];
...
inline int minloc(const int n, vector<double> dat){
	int imin = 0;
	for (int i=1; i<n; i++){
...
inline int maxloc(const int n, vector<double> dat){
	int imax = 0;
	for (int i=1; i<n; i++){
...
template <class A> inline void build_2D_array(vector<vector <A>> & array, int Narray1, int Narray2){
